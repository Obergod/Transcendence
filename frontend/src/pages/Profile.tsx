...
const Profile = ({ onLogout }: { onLogout: () => void }) => {
  ...
  // 1. On récupère le VRAI utilisateur et la fonction de déconnexion du contexte !
  const { user, logout } = useUser();

  // 2. On initialise le champ modifiable avec le vrai pseudo
  const [pseudo, setPseudo] = useState(user?.pseudo || "");
  const [isEditing, setIsEditing] = useState(false);

  // NOTRE FAUSSE BASE DE DONNÉES D'AMIS (On la garde en attendant que le back la gère)
  ...
  const handleLogout = () => {
    logout(); // On vide le contexte React
    onLogout(); // On prévient App.tsx
    navigate("/"); // On retourne à l'accueil
  };

  // Sécurité : si l'utilisateur n'est pas chargé, on ne rend rien (ou on redirige)
  if (!user) return null;
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-12">
            ...
            <div className="flex-1 w-full space-y-6">
              <div>
                ...
                <div className="flex gap-3">
                  <input
                    ...
                    className={`flex-1 bg-gray-900 border ${isEditing ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-600'} rounded-xl px-4 py-3 focus:outline-none transition-all text-white`}
                  />
                  <button
                    onClick={() => setIsEditing(!isEditing)}
                    className="bg-gray-700 hover:bg-gray-600 px-6 rounded-xl font-bold transition-colors text-white"
                  >
              ...
              <div>
                ...
                {/* On affiche le VRAI email ici ! */}
                <input type="text" disabled value={user.email} className="w-full bg-gray-900/50 border border-gray-700 rounded-xl px-4 py-3 text-gray-500 cursor-not-allowed" />
              </div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto pr-2">
            {friends.map((friend) => (
              <div key={friend.id} className="flex items-center justify-between p-3 bg-gray-900/80 rounded-xl border border-gray-700 hover:border-gray-500 transition-colors">
                ...
                <Link to={`/chat/${friend.pseudo}`} className="text-gray-400 hover:text-red-500 transition-colors p-2">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
